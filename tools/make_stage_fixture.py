...
Usage:  python3 tools/make_stage_fixture.py N > src/stage_test.asm
...
def real_metatile_defs():
    # Always take the tileset from the committed stage_test.asm so a truncated /
    # fixture working copy cannot corrupt it.
    text = subprocess.check_output(
        ['git', '-C', str(ROOT), 'show', 'HEAD:src/stage_test.asm'], text=True)
...
def main():
    ...
    out.append('// Not for commit - restore src/stage_test.asm from git after testing.')
    out.append('// ============================================================================')
    out.append('')
