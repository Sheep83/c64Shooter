...
TUR=src/generated/stage_turrets.asm

# Back up and restore the generated level files (they own STAGE_METATILE_ROWS,
# the stage map, and turret placement). Restore from a byte copy, not git, so
# this works whether or not the generated files are committed yet.
...
# Minimal in-range turret placement for the synthetic fixture (the widen probe
# does not care about turrets; this just satisfies the placement guards for any N).
printf '.const TURRET_COUNT = 1\n.var turretCols = List().add(1)\n.var turretRows = List().add(1)\n' > "$TUR"
