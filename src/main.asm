...
// A monochrome bas-relief hull surface (Uridium-style depth from a restricted
// palette). Light source is top-left throughout the stage:
//   bit pair 00 -> $D021 = 0  black       DEEP recesses / grooves / vents / shadow
//   bit pair 01 -> $D022 = TERRAIN_MC_COLOUR_1 (12, grey)  DOMINANT BASE PLATING
//   bit pair 10 -> $D023 = TERRAIN_MC_COLOUR_2 (15, lt grey)  raised faces / bevels
//   bit pair 11 -> TERRAIN_COLOUR_RAM & 7 (1, white)  highest edges / highlights
// There is a visible grey surface everywhere: black describes depth, never the
// absence of terrain.
.const TERRAIN_MC_COLOUR_1 = 12                     // $D022 shared shade: grey (the base surface).
.const TERRAIN_MC_COLOUR_2 = 15                     // $D023 shared shade: light grey (raised/bevel).
.const TERRAIN_COLOUR_RAM  = 8 | 1                  // Fixed colour RAM for all terrain cells:
                                                     // bit 3 = multicolour, low 3 bits = white (1).
...
.const STAGE_METATILE_ROWS = 25                      // Metatile rows in the shipped stage_test.asm. MUST match
                                                      // the generated stageMetatileRows byte count (hard guard
                                                      // after the #import). decodeStageCharacterRow now uses
                                                      // 16-bit logical-row / metatile-row / stage-map-offset
                                                      // arithmetic, so this is bounded by memory, not by an
                                                      // 8-bit index. See docs/stage-widening-worklog.md.
.const STAGE_LOGICAL_ROWS = STAGE_METATILE_ROWS * METATILE_H // Total logical character rows.
...
.const TERRAIN_GLYPH_COUNT = 48                     // Glyphs actually authored/copied so far (<= namespace).
...
!rowLoop:
    stx BG_DEST_ROW
    jsr renderStageRowToScreen              // Character row only; colour RAM is the fixed fill above.
    ldx BG_DEST_ROW
    inx
    cpx #24
    bne !rowLoop-
...
// value is < 2*STAGE_LOGICAL_ROWS, so a single conditional subtraction is
// enough. 16-bit compare + 16-bit subtract; no 8-bit truncation. Clobbers A.
wrapBgLogicalRow:
    lda BG_LOGICAL_ROW_HI
...
// namespace -> $3D00.. ; 48 authored (codes 160..207, $3D00..$3E7F), 208..223
// free. 8x8 multicolour-text bitmaps: each byte = 4 double-wide pixels, bit
// pair 00 = black (depth), 01 = grey (base plating), 10 = light grey (raised),
// 11 = white (highlight). Copied into the RAM charset once per game start by
// initBackground. This is a construction kit for a continuous grey hull
// surface: base plating + seams, raised-platform face/rims/corners, channels,
// inset panels, grilles, machinery detail, transitions. Light source top-left.
// (Auto-generated by scratchpad/relief.py; see docs/multicolour-terrain-worklog.md.)
terrainGlyphs:
    .byte $55,$55,$55,$55,$55,$55,$55,$55   // 160 PLATE   base plating (grey)
    .byte $55,$55,$55,$55,$55,$51,$11,$55   // 161 PL_SB   base + recessed bottom seam
    .byte $54,$55,$55,$55,$54,$55,$55,$55   // 162 PL_SR   base + recessed right seam
    .byte $54,$55,$55,$55,$50,$51,$11,$55   // 163 PL_SC   base + bottom+right seam corner
    .byte $55,$65,$55,$54,$55,$59,$55,$45   // 164 PL_TX   base + faint machined texture
    .byte $55,$51,$79,$6d,$45,$55,$55,$55   // 165 PL_ST   base + single bevelled stud
    .byte $aa,$a6,$aa,$9a,$aa,$a6,$9a,$aa   // 166 R_FIL   raised platform top face
    .byte $aa,$be,$be,$aa,$aa,$aa,$aa,$aa   // 167 R_HI    raised face + bright boss
    .byte $aa,$be,$be,$aa,$aa,$96,$96,$aa   // 168 R_PL    raised face + boss + inscribed panel
    .byte $55,$55,$ff,$aa,$aa,$aa,$aa,$aa   // 169 R_T     platform north lit rim
    .byte $aa,$aa,$aa,$aa,$aa,$a5,$40,$55   // 170 R_B     platform south step-down + shadow -> base
    .byte $ea,$ea,$ea,$ea,$ea,$ea,$ea,$ea   // 171 R_L     platform west lit rim
    .byte $a5,$a5,$a5,$a5,$a5,$a5,$a5,$a5   // 172 R_R     platform east step-down -> base
    .byte $55,$55,$ff,$ea,$ea,$ea,$ea,$ea   // 173 R_TL    platform NW corner
    .byte $55,$55,$ff,$a5,$a5,$a5,$a5,$a5   // 174 R_TR    platform NE corner
    .byte $ea,$ea,$ea,$ea,$ea,$a5,$40,$55   // 175 R_BL    platform SW corner
    .byte $a5,$a5,$a5,$a5,$a5,$a5,$40,$55   // 176 R_BR    platform SE corner
    .byte $53,$53,$53,$53,$53,$53,$53,$53   // 177 TV_L    vertical channel: base | groove | lit far wall
    .byte $c5,$c5,$c5,$c5,$c5,$c5,$c5,$c5   // 178 TV_R    vertical channel: lit near wall | groove | base
    .byte $55,$55,$55,$55,$55,$ff,$00,$00   // 179 TH_T    horiz groove: base + lit near lip -> floor
    .byte $00,$00,$aa,$55,$55,$55,$55,$55   // 180 TH_F    horiz groove: floor -> lit far wall -> base
    .byte $55,$55,$55,$55,$55,$70,$40,$55   // 181 TH_L    horiz groove left end
    .byte $55,$55,$55,$55,$55,$0d,$01,$55   // 182 TH_R    horiz groove right end
    .byte $55,$55,$55,$55,$55,$55,$00,$00   // 183 RC_T    inset panel: top inner wall (shadow)
    .byte $ff,$aa,$55,$55,$55,$55,$55,$55   // 184 RC_B    inset panel: bottom inner wall (lit lip)
    .byte $50,$50,$50,$50,$50,$50,$50,$50   // 185 RC_L    inset panel: left inner wall (shadow)
    .byte $3d,$3d,$3d,$3d,$3d,$3d,$3d,$3d   // 186 RC_R    inset panel: right inner wall (lit)
    .byte $55,$55,$55,$55,$55,$55,$55,$55   // 187 RC_F    inset panel: recessed floor (grey)
    .byte $55,$55,$ff,$aa,$41,$aa,$41,$aa   // 188 GR_T    grille top frame + raised louvres
    .byte $aa,$41,$aa,$41,$aa,$41,$aa,$41   // 189 GR_M    grille louvre body (lt-grey slats, dark grooves)
    .byte $aa,$41,$ff,$55,$55,$55,$55,$55   // 190 GR_B    grille bottom frame -> base
    .byte $5e,$5e,$5e,$5e,$5e,$5e,$5e,$5e   // 191 GR_L    grille left frame column
    .byte $b5,$b5,$b5,$b5,$b5,$b5,$b5,$b5   // 192 GR_R    grille right frame column
    .byte $aa,$be,$41,$aa,$be,$41,$aa,$be   // 193 MC_HP   machinery: stacked horizontal pipes
    .byte $b5,$b5,$b5,$b5,$b5,$b5,$b5,$b5   // 194 MC_VP   machinery: vertical pipe
    .byte $aa,$eb,$aa,$aa,$eb,$aa,$aa,$eb   // 195 MC_BT   machinery: bolt rows
    .byte $aa,$aa,$41,$41,$aa,$82,$aa,$aa   // 196 MC_SL   machinery: housing with dark slot
    .byte $55,$d5,$b5,$ad,$ab,$aa,$aa,$aa   // 197 DG_R    base -> raised, lit "/" bevel
    .byte $55,$57,$5e,$7a,$ea,$aa,$aa,$aa   // 198 DG_L    base -> raised, lit "\" bevel
    .byte $55,$ff,$aa,$55,$55,$ff,$aa,$55   // 199 STEP    two lit step treads on base
    .byte $55,$d7,$7d,$75,$75,$7d,$d7,$55   // 200 HAZ     hazard chevrons on base
    .byte $00,$00,$00,$00,$00,$00,$00,$00   // 201 (reserved for editor output)
    .byte $00,$00,$00,$00,$00,$00,$00,$00   // 202 (reserved)
    .byte $00,$00,$00,$00,$00,$00,$00,$00   // 203 (reserved)
    .byte $00,$00,$00,$00,$00,$00,$00,$00   // 204 (reserved)
    .byte $00,$00,$00,$00,$00,$00,$00,$00   // 205 (reserved)
    .byte $00,$00,$00,$00,$00,$00,$00,$00   // 206 (reserved)
    .byte $00,$00,$00,$00,$00,$00,$00,$00   // 207 (reserved)
terrainGlyphsEnd:
