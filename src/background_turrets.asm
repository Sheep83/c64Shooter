...
* = $8800
// TURRET_COUNT and the turretCols/turretRows placement lists are GENERATED by
// the level editor (src/generated/stage_turrets.asm, #imported near the top of
// main.asm). This file owns turret BEHAVIOUR and validates that placement data.
.if (TURRET_COUNT < 1 || TURRET_COUNT > 3) {
    .error "TURRET_COUNT must be 1..3 (private turret glyph namespace 226..239)"
}
...
// Editor-generated placement layer: turretCols (world CHARACTER columns) and
// turretRows (world CHARACTER / logical rows, full 16-bit stage height) come
// from src/generated/stage_turrets.asm. A body covers 2x2 cells; its second row
// wraps modulo STAGE_LOGICAL_ROWS. The editor places turrets on the metatile
// grid, so each authored position is metatileIndex*4 + 1 - normally the
// interior of an M14 MACH housing, so a turret reads as machinery emerging from
// the hull surface.
.if (turretCols.size() != TURRET_COUNT || turretRows.size() != TURRET_COUNT) {
    .error "Generated turret placement count does not match TURRET_COUNT"
}
